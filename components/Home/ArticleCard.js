...
function ArticleCard({ title, owner }) {
  ...
  useEffect(() => {
    ...
    setArticle(storedList ? JSON.parse(storedList) : []);
  }, []);
  const handleReadMore = (e) => {
    router.push(`article/${e.target.value}`);
  };
  const handleEdit = (e) => {
    router.push(`article/${e.target.value}/edit`);
  };
  const handleDelete = (e) => {
    const idx = e.target.value - 1;
    const getArticle = JSON.parse(localStorage.getItem("articleList"));
    getArticle.splice(idx, 1);
    localStorage.setItem("articleList", JSON.stringify(getArticle));
      ...
      <div className="m-auto grid centerGrid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 px-[2vw] lg:px-[5vw]">
        {article.length > 0 ? (
          article.map((card, index) => {
            return (
              <div
                className="flex flex-col flex-wrap justify-between items-center w-[85vw] md:w-[43vw] lg:w-[26vw] p-[40px] mb-8 shadow-md hover:shadow-xl duration-300 shadow-gray-400"
                key={index}
              >
                <h1 className="text-2xl font-semibold">{card.title}</h1>
                <Image
                  className="mt-3.5 h-[30vh] object-cover"
                  src={card.image}
                  alt=""
                  width={320}
                  height={100}
                ...
                <div className="w-[95%] flex justify-between items-center mb-3.5">
                  <span className="text-sm font-serif">{card.author}</span>
                  <span className="text-xs">{card.date}</span>
                </div>
                <p className="text-justify">
                  {card.article.substring(0, 200)}...
                </p>
          ...
          <div className="mb-5">No Article</div>
        )}
      </div>
    </div>
  );
